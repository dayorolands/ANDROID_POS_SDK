        ...
        fun bind(txn: TransactionLog?){
            txn?.toResult()?.apply {
                ...
                itemView.setOnClickListener {
                    // create intent to show transaction details
                    val intent = Intent(tvAmount.context, TransactionDetailActivity::class.java)
                        .putExtra(TransactionDetailActivity.KEY_TRANSACTION, this)

                    // show details for transaction
                    tvAmount.context.startActivity(intent)
                }
            }
        }
    }
